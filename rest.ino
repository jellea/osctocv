  ...
  if (configuration.restEnabled) {
    webServer.on("/out", []() {
      ...
      if (i > 0 && i <= 20) {
        if (modee == "trig") {
          if (value > 1 || value < 0) {
            webServer.send(200, textHMTL, valueMustBeMinusOneOne);
          } else {
          ...
          if (value > 1 || value < 0) {
            webServer.send(200, textHMTL, valueMustBeZeroOne);
          } else {
          ...
          webServer.send(200, textHMTL, outputModeError);
        }

      } else {
        webServer.send(200, textHMTL, indexError);
      }
    ...
    webServer.on("/in", []() {
      ...
      if (i > 0 && i <= 20) {
        if (modee == "trig") {
          ...
          webServer.send(200, textHMTL, inputModeError);
        }
      } else {
        webServer.send(200, textHMTL, indexError);
      }
    ...
    webServer.on("/wifi", []() {
      ...
      if (modee == "ap" || modee  == "client") {
        ...
        webServer.send(200, textHMTL, okReboot);
      } else {
