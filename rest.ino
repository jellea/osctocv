...
void createWebServerRoutes() {
  ...
  webServer.on("/out", []() {
    ...
    if (i > 0 && i <= 20) {
      ...
      webServer.send(200, "text/plain", "index must be between 1 and 20");
    }
  ...
  webServer.on("/in", []() {
    ...
    if (i > 0 && i <= 20) {
      if (modee == "trig") {
        ...
        webServer.send(200, "text/plain", "mode must be one of trig, gate, cvuni, cvbi");
      }
  ...
  webServer.on("/wifi", []() {
    ...
    if (modee == "client") {
      ...
      webServer.send(200, "text/plain", "mode must be one of client, ap.");
    }
  });
