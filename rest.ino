...
const char statusHTML[] PROGMEM = "<html>\n<title>OSCPixi</title>\n<link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css' crossorigin='anonymous'>\n<div class='container' style='width:450px;margin20px;padding:20px'>\n \n  <nav class='navbar navbar-default'>\n   <div class='container-fluid'>\n     <div class='collapse navbar-collapse'>\n        <ul class='nav navbar-nav'>\n       <li><a href='/'>OSCPixi</a><li>\n       <li><a href='/rest'>Rest API</a></li>\n       <li><a href='/wifi'>WLAN</a></li>\n       <li class='active'><a href='/status'>Status</a></li>\n        </ul>\n     </div>\n    </div>\n  </nav>\n\n  \n  <iframe src='/status/data' name='' style='border:none;width:350px;height:400px;'></iframe>\n  \n  \n</div>\n<html>";
...
void createWebServerRoutes() {
  ...
  if (configuration.restEnabled) {
      ...
      if (channel > 0 && channel <= 20) {
        if (modee == "trig") {
          if (value > 1 || value < 0) {
            ...
            setChannel(channel - 1, OUTPUT_MODE_TRIG, value);
            webServerSendOK();
          }
        } else  if (modee == "gate") {
          if (value > 1 || value < 0) {
            webServer.send_P(400, textHMTL, valueMustBeZeroOne);
          } else {
            setChannel(channel - 1, OUTPUT_MODE_GATE, value);
            webServerSendOK();
          }
          ...
          if (value > 1 || value < 0) {
            ...
            setChannel(channel - 1, OUTPUT_MODE_FLIPFLOP, value);
            webServerSendOK();
          }
        } else  if (modee == "cvuni") {
          if (value > 1 || value < 0) {
            ...
            setChannel(channel - 1, OUTPUT_MODE_CVUNI, value);
            webServerSendOK();
          }
        } else  if (modee == "cvbi") {
          if (value > 1 || value < -1) {
            ...
            setChannel(channel - 1, OUTPUT_MODE_CVBI, value);
            webServerSendOK();
          }

        } else  if (modee == "lfoSine") {
          setChannel(channel - 1, OUTPUT_MODE_LFO_SINE, 0);
          configuration.channelLFOFrequencies[channel - 1] = value;
          webServerSendOK();
        } else  if (modee == "lfoSaw") {
          setChannel(channel - 1, OUTPUT_MODE_LFO_SAW, 0);
          configuration.channelLFOFrequencies[channel - 1] = value;
          webServerSendOK();
        } else  if (modee == "lfoRamp") {
          setChannel(channel - 1, OUTPUT_MODE_LFO_RAMP, 0);
          configuration.channelLFOFrequencies[channel - 1] = value ;
          webServerSendOK();
        } else  if (modee == "lfoTri") {
          setChannel(channel - 1, OUTPUT_MODE_LFO_TRI, 0);
          configuration.channelLFOFrequencies[channel - 1] = value ;
          webServerSendOK();
        } else  if (modee == "lfoSquare") {
          setChannel(channel - 1, OUTPUT_MODE_LFO_SQUARE, 0);
          configuration.channelLFOFrequencies[channel - 1] = value;
          webServerSendOK();
        } else {
    ...
    webServer.on("/in", []() {
      String modee =  webServer.arg("mode");
      //String ret =  webServer.arg("return");
      int channel = webServer.arg("channel").toInt();

      if (channel > 0 && channel <= 20) {
        if (modee == "trig") {
          setChannel(channel - 1, INPUT_MODE_TRIG, 0);
          webServerSendOK();
        } else  if (modee == "gate") {
          setChannel(channel - 1, INPUT_MODE_GATE, 0);
          webServerSendOK();
        } else  if (modee == "cvuni") {
          setChannel(channel - 1, INPUT_MODE_CVUNI, 0);
          webServerSendOK();
        } else  if (modee == "cvbi") {
          setChannel(channel - 1, INPUT_MODE_CVBI, 0);
          webServerSendOK();
        } else {
